...
def run(
    ...
    if valid_data is None:
        ...
        eids_list = np.random.RandomState(seed=42).permutation(
            eids,
        )
    ...
    for e in range(n_epochs):
        ...
        train_auc, train_acc, train_loss = train(
            train_data=train_data, model=model, optimizer=optimizer, id2index=id2index
        )
...
def train(
    model: nn.Module, train_data: dict, optimizer: torch.optim.Optimizer, id2index: dict
):
